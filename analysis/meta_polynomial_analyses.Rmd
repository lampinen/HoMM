...
result_subdirs = c("basic_ADAM_nobinary_slower", "untrained_baseline")
...
name_run_type = function(run_type) { # names for plotting
  case_when(drun_type == "basic_ADAM_nobinary_slower" ~ "EML",
            ...
            run_type == "nometa" ~ "EML (no meta tasks)",
...
#                   group_by_at(vars(one_of(x_var, color_var))) %>%
                   group_by_at(vars(one_of(x_var))) %>% 
                   summarize_at(y_var, mean, na.rm=T),
#                 aes_string(yintercept=y_var, color=color_var),
...
process_meta_d = function(meta_d) {
  meta_d = meta_d %>%
    ...
    ungroup() %>%
    mutate(result_type=case_when(!is_new_argument & !is_new_meta_mapping ~ "Trained mapping, trained task",
                          is_new_argument & !is_new_meta_mapping ~ "Trained mapping, held-out task",
                          !is_new_argument & is_new_meta_mapping ~ "Held-out mapping, trained task",
                          is_new_argument & is_new_meta_mapping ~ "Held-out mapping, held-out task"),
           result_type = factor(result_type, levels = c('Trained mapping, trained task', 'Trained mapping, held-out task', 'Held-out mapping, trained task', 'Held-out mapping, held-out task')))
  return(meta_d)
}
```
...
```{r}
summary_plot(meta_d %>%
               filter(!(run_type %in% c("basic_more", "language"))),
...
```{r}
summary_plot(meta_d %>%
               filter(run_type %in% c("untrained_baseline", "basic_ADAM_nobinary_slower", "language_ADAM")),
             "named_run_type", "loss", "result_type") +
  ylim(0, NA) +
  labs(y="Performance after meta-mapping") +
  geom_hline(yintercept=0)
ggsave("../../meta_RL/writing/figures/poly/meta_results.png", width=6, height=3)
```
