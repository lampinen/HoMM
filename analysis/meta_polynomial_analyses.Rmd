...
name_run_type = function(run_type) { # names for plotting
  case_when(run_type == "basic_ADAM_nobinary_slower" ~ "EML",
            ...
            run_type == "basic_separate_meta" ~ "EML (unshared Z)",
...
```{r}
learning_meta_d = load_d(results_dir, c("basic_ADAM_nobinary_slower", "basic_separate_meta", "language_meta_only", "untrained_baseline"), num_runs, "meta_true_losses") %>% 
  filter(epoch %% 200 == 0) 
```
...
```{r}
summary_plot(learning_meta_d %>%
               filter(! (run_type %in% c("untrained_baseline", "basic_separate_meta"))) %>%
               mutate(named_run_type = ifelse(named_run_type == "Language", "EML (+ language)", named_run_type),
                      log_loss = log(loss)),
             "epoch", "loss", "named_run_type",
             continuous_x = T) +
